...
private:
	...
	{
		auto& DataItemParameters = Context.Items.QueryData.DataItemParameters;
		if (DataItemParameters.Num() == PreviewValues.Num())
		{
			for (int32 Index = 0; Index < OutputCount; Index++)
			{
				Outputs[Index].template Get<v_flt>() = DataItemParameters.GetData()[Index];
			}
		}
		else
		{
	...
	{
		Constructor.AddLinef(TEXT("if (%s.Items.QueryData.DataItemParameters.Num() == %d)"), *FVoxelCppIds::Context, PreviewValues.Num());
		Constructor.StartBlock();
		for (int32 Index = 0; Index < OutputCount; Index++)
		{
			Constructor.AddLinef(TEXT("%s = %s.Items.QueryData.DataItemParameters.GetData()[%d];"), *Outputs[Index], *FVoxelCppIds::Context, Index);
		}
