...
static bool CompileInternal(UVoxelGraphGenerator* Generator, bool bIsAutomaticCompile, FString& OutError)
{
	...
	// Show the file browser dialog
	{
		...
		{
			if (Generator->bBuiltinPluginGenerator)
			{
				HeaderPath = FPaths::ConvertRelativePathToFull(Plugin.GetBaseDir() / Generator->SaveLocation.FilePath);
			}
			else
			{
				HeaderPath = FPaths::ConvertRelativePathToFull(FPaths::ProjectDir() / Generator->SaveLocation.FilePath);
			}
		}
		else
		{
			const auto Result = FMessageDialog::Open(EAppMsgType::YesNoCancel, VOXEL_LOCTEXT("Do you want to choose a custom header path?\nIf not familiar with C++ or unsure, choose No"));
			...
			if (Result == EAppReturnType::Yes)
			{
					...
					OutFiles))
				{
					...
					HeaderPath = FPaths::ConvertRelativePathToFull(OutFiles[0]);
				}
				else
				{
					Popup("Canceled", false);
					return true;
				}
			}
			else if (Result == EAppReturnType::No)
			{
				HeaderPath = FPaths::ConvertRelativePathToFull(GeneratedDirectory / Generator->GetName() + ".h");
			}
			else
			{
				Popup("Canceled", false);
		...
		Generator->SaveLocation.FilePath = HeaderPath;
		if (FPaths::IsUnderDirectory(HeaderPath, Plugin.GetBaseDir()))
		{
			Generator->bBuiltinPluginGenerator = true;
			ensure(FPaths::MakePathRelativeTo(Generator->SaveLocation.FilePath, *(Plugin.GetBaseDir() + TEXT("/"))));

			if (VOXEL_PLUGIN_NAME != FString("Voxel"))
			{
				FMessageDialog::Open(EAppMsgType::Ok, VOXEL_LOCTEXT("This is a builtin generator asset.\nTo modify it, copy it to your project and clear the 'Save Location' property in the graph settings"));
			}
		}
		else
		{
			Generator->bBuiltinPluginGenerator = false;
			ensure(FPaths::MakePathRelativeTo(Generator->SaveLocation.FilePath, *FPaths::ProjectDir()));
		}
		
		if (!Generator->GeneratedCppClass.IsValid())
		{
			if (Generator->bBuiltinPluginGenerator)
			{
				Generator->GeneratedCppClass = FSoftClassPath("/Script/VoxelGraphs." + BaseName);
			}
			else
			{
				Generator->GeneratedCppClass = FSoftClassPath("/Script/GeneratedWorldGenerators." + BaseName);
			}

			// If no existing class, prompt for restart
		...
		if (!Generator->bUseCppClassInsteadOfGraph)
		{
			Popup("If you want to use the generated C++ class, you can tick 'Use C++ class instead of graph' in the graph options");
		}
