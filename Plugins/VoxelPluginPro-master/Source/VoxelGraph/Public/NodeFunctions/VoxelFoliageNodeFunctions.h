...
namespace FVoxelFoliageNodeFunctions
{
	...
	inline int32 InitBiome(const FVoxelGeneratorInit& InitStruct, TWeakObjectPtr<UVoxelFoliageBiomeBase> Biome)
	{
		...
		const TVoxelSharedPtr<IVoxelFoliageInterface> FoliageInterface = Runtime->GetSubsystem<IVoxelFoliageInterface>();
		return FoliageInterface ? FoliageInterface->RegisterBiome(Biome.Get()) : -1;
	}
}