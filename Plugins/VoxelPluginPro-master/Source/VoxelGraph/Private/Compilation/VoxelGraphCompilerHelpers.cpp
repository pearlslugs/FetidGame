...
public:
	...
	TVoxelPassthroughCompilationNode(const FVoxelGraphContext& Context, const TArray<const UVoxelNode*>& InSourceNodes)
		: Parent(Context, *GetDefault<UVoxelNode>(), TArray<EVoxelPinCategory>({ Category }), TArray<EVoxelPinCategory>({ Category }))
	{
...
TSharedRef<FVoxelCompilationNode> FVoxelGraphCompilerHelpers::GetPassthroughNode(EVoxelPinCategory Category, const FVoxelGraphContext& Context, const TArray<const UVoxelNode*>& SourceNodes)
{
	switch (Category)
	{
	case EVoxelPinCategory::Exec:
		return MakeShared<FVoxelExecPassthrough>(Context, SourceNodes);
	case EVoxelPinCategory::Boolean:
		return MakeShared<FVoxelBoolPassthrough>(Context, SourceNodes);
	case EVoxelPinCategory::Int:
		return MakeShared<FVoxelIntPassthrough>(Context, SourceNodes);
	case EVoxelPinCategory::Float:
		return MakeShared<FVoxelFloatPassthrough>(Context, SourceNodes);
	case EVoxelPinCategory::Material:
		return MakeShared<FVoxelMaterialPassthrough>(Context, SourceNodes);
	case EVoxelPinCategory::Color:
		return MakeShared<FVoxelColorPassthrough>(Context, SourceNodes);
	case EVoxelPinCategory::Seed:
		return MakeShared<FVoxelSeedPassthrough>(Context, SourceNodes);
	default:
		check(false);
...
FVoxelCompilationNode* FVoxelGraphCompilerHelpers::AddPassthrough(FVoxelGraphCompiler& Compiler, FVoxelCompilationPin& Pin, FVoxelCompilationNode* SourceNode)
{
	...
	const auto Passthrough = GetPassthroughNode(Pin.PinCategory, SourceNode->Context, SourceNode->SourceNodes);
