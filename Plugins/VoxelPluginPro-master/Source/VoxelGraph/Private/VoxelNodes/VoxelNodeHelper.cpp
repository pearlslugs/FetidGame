...
TSharedPtr<FVoxelCompilationNode> UVoxelSetterNode::GetCompilationNode(const FVoxelGraphContext& Context) const
{
	auto Result = MakeShared<FVoxelSetterCompilationNode>(Context, *this);
	Result->OutputIndex = GetOutputIndex();
	return Result;
}

TSharedPtr<FVoxelCompilationNode> UVoxelPureNode::GetCompilationNode(const FVoxelGraphContext& Context) const
{
	return MakeShared<FVoxelDefaultPureCompilationNode>(Context, *this);
}

TSharedPtr<FVoxelCompilationNode> UVoxelNodeWithDependencies::GetCompilationNode(const FVoxelGraphContext& Context) const
{
	auto Result = MakeShared<FVoxelAxisDependenciesCompilationNode>(Context, *this);
