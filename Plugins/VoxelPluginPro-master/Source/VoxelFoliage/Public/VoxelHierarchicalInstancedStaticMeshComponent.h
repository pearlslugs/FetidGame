...
public:
	...
	const FVoxelInstancedMeshKey& Voxel_GetMeshAndActorSettings() const
	{
		return Voxel_MeshKey;
	}

private:
	TVoxelWeakPtr<FVoxelPool> Voxel_Pool;
...
public:
	TVoxelWeakPtr<FVoxelInstancesSection> Voxel_AppendTransforms(
	...
	void Voxel_EnablePhysics(const FVoxelIntBox& Chunk);
	void Voxel_DisablePhysics(const FVoxelIntBox& Chunk);
	void Voxel_RefreshPhysics(const FVoxelIntBox& BoundsToUpdate);

	FVoxelFoliageTransforms Voxel_RemoveInstancesInArea(
		const FVoxelIntBox& VoxelBounds, 
		const FVoxelConstDataAccelerator* Accelerator, 
		EVoxelSpawnerActorSpawnType SpawnType);
	bool Voxel_RemoveInstanceByIndex(int32 InstanceIndex, FVoxelFoliageTransform& OutTransform);
...
private:
	...
	TArray<FVoxelFoliageMatrix> Voxel_UnbuiltMatrices;
	TArray<FVoxelFoliageMatrix> Voxel_BuiltMatrices;
	TArray<float> Voxel_CustomData;
	...
	struct FVoxelMappings
	{
		...
		struct FDeletion
		{
			FVoxelHISMBuildTaskId TaskUniqueId; // Id of the task started right after that deletion
			int32 StartIndex = -1;
			int32 Num = -1;
		};
	...
	FVoxelHISMBuildTaskId Voxel_TaskUniqueId;
	...
	TMap<FVoxelHISMBuildTaskId, TArray<FVoxelIntBox>> Voxel_TaskIdToNewInstancesBounds;
	...
	struct FBodyWithTransform
	{
		FBodyInstance* BodyInstance = nullptr;
		FTransform LocalTransform;
	};
	TMap<FVoxelIntBox, TArray<FBodyWithTransform>> Voxel_InstanceBodies;
	...
	void Voxel_EnablePhysicsImpl(const FVoxelIntBox& Chunk, TArray<FBodyWithTransform>& OutBodies) const;
	void Voxel_DisablePhysicsImpl(TArray<FBodyWithTransform>& Bodies) const;
	...
	FORCEINLINE FVoxelVector Voxel_GetGlobalVoxelPosition(const FVoxelFoliageMatrix& Matrix) const
	{
