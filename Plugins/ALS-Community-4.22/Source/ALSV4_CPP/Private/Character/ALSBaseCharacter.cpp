	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<UALSCharacterMovementComponent>(CharacterMovementComponentName))
{
	...
	//combat init
	bIsAttacking = false;
	bISCombatMovement = false;
	bIsCombatStepping = false;
	bShouldParry = false;
...
void AALSBaseCharacter::Tick(float DeltaTime)
{
	...
	SetEssentialValues(DeltaTime);
		...
		//if (bISCombatMovement || bIsLockedOn)
		//{
		//	if (Gait != EALSGait::Walking && MovementState != EALSMovementState::Ragdoll)
		//	{
		//		SetDesiredGait(EALSGait::Walking);
		//		SetGait(EALSGait::Walking, true);
		//		GetCharacterMovement()->SetMovementMode(MOVE_Walking);
		//	}
		//}

	MyCharacterMovementComponent->SetIsInjured(bIsInjured);
		if (MovementState == EALSMovementState::Grounded)
		{
			//follow the injured changes and apply them to locked on, it should go from inbetween run and jog to jog when you press stprint
...
bool AALSBaseCharacter::CanSprint() const
{
	...
	if (!bHasMovementInput || RotationMode == EALSRotationMode::Aiming || bIsAttacking || bIsLockedOn || bISCombatMovement)
	{
		return false;
	}
...
void AALSBaseCharacter::OnViewModeChanged(const EALSViewMode PreviousViewMode)
{
	...
	{
		if (RotationMode == EALSRotationMode::VelocityDirection || RotationMode == EALSRotationMode::LookingDirection || ViewMode == EALSViewMode::FirstPerson)
		{
...
void AALSBaseCharacter::UpdateGroundedRotation(float DeltaTime)
{
	if (MovementAction == EALSMovementAction::None)
	{
		...
		if (bCanUpdateMovingRot)
		{
			...
			else if (RotationMode == EALSRotationMode::LookingDirection || RotationMode == EALSRotationMode::Aiming && false)
			{
...
void AALSBaseCharacter::PlayerRightMovementInput(float Value)
{
	if (!bIsAttacking)
	{
		if (MovementState == EALSMovementState::Grounded || MovementState == EALSMovementState::InAir && !bIsAttacking)
		{
			// Default camera relative movement behavior
...
{
	if (bIsLockedOn && bISCombatMovement)
	{
...
void AALSBaseCharacter::StopAttacking()
{
	bIsAttacking = false;
	DeltaWalkCurve = 0;
}

void AALSBaseCharacter::AssessTarget(AActor* Target)
{
	AALSBaseCharacter* CastedTarget = Cast<AALSBaseCharacter>(Target);
	if (CastedTarget)
	{
		CombatHumanoidTarget = CastedTarget;
	}
	CombatTarget = Target;
...
bool AALSBaseCharacter::IsHand(FString BoneName)
{
	if (BoneName.Contains(FString("index")) || BoneName.Contains(FString("middle")) || BoneName.Contains(FString("thumb")) || BoneName.Contains(FString("ring")) || BoneName.Contains(FString("pinky")))
	{
		return true;
	}
	return false;
...
FString AALSBaseCharacter::SelectBodyPart(FName BoneName, FAttackType IncomingAttack)
{
	FString BoneNameToStringLower = BoneName.ToString();
	BoneNameToStringLower = BoneNameToStringLower.ToLower();
	FString Position;
	FString BodyPart;
	
		for (auto  BoneNameFromList : BoneList)
		{
			if (BoneName == BoneNameFromList)
			{
				
				if (BoneNameToStringLower.Contains(FString("arm")) || BoneNameToStringLower.Contains(FString("hand")) || BoneNameToStringLower.Contains(FString("clavicle")) || IsHand(BoneNameToStringLower))
				{
					Position = GetLeftOrRight(BoneNameToStringLower);
					BodyPart = "Arm";
					BodyPart = Position.Append(BodyPart);
					return BodyPart;
				}
				if (BoneNameToStringLower.Contains("calf") || BoneNameToStringLower.Contains("foot") || BoneNameToStringLower.Contains("thigh"))
				{
					Position = GetLeftOrRight(BoneNameToStringLower);
					BodyPart = "Leg";
					BodyPart = Position.Append(BodyPart);
				...
				if (BoneNameToStringLower.Contains("spine") || BoneNameToStringLower.Contains("pelvis"))
				{
					BodyPart = "Torso";
					return BodyPart;
				}
				if (BoneNameToStringLower.Contains("head") || BoneNameToStringLower.Contains("neck"))
				{
					BodyPart = "Head";
					return BodyPart;
				}
...
FString AALSBaseCharacter::GetLeftOrRight(FString Position)
{
	if (Position.Contains("_l"))
	{
		return "Left";
	}
	if (Position.Contains("_r"))
	{
		return "Right";
	}
	return "Error";
...
void AALSBaseCharacter::SetBoneListFromNewBoneName(FName NewBoneName)
{

	if (NewBoneName.IsValid())
	{
		BoneList.Add(NewBoneName);
	}
}