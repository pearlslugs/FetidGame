...
{
	auto* ClampNode = Compiler.AddNode(GetDefault<UVoxelNode_Clamp>()->GetCompilationNode(Compiler.Context));
	...
	auto* EqualNode = Compiler.AddNode(GetDefault<UVoxelNode_FLessEqual>()->GetCompilationNode(Compiler.Context));
	...
	auto* IfNode = Compiler.AddNode(GetDefault<UVoxelNode_IfWithDefaultToFalse>()->GetCompilationNode(Compiler.Context));
	...
	auto* MultiplyNode = Compiler.AddNode(GetDefault<UVoxelNode_FMultiply>()->GetCompilationNode(Compiler.Context));
	...
	auto* AddNode = Compiler.AddNode(GetDefault<UVoxelNode_FAdd>()->GetCompilationNode(Compiler.Context));
	...
	auto* FlowMergeNode = Compiler.AddNode(GetDefault<UVoxelNode_FlowMerge>()->GetCompilationNode(Compiler.Context));
...
void FVoxelReplaceBiomeMergePass::Apply(FVoxelGraphCompiler& Compiler)
{
	for (auto* Node : Compiler.GetAllNodesCopy())
	{
		if (auto* BiomeMergeNode = CastVoxel<FVoxelBiomeMergeCompilationNode>(Node))
		{
			...
			auto* SumInputPassthrough = Compiler.AddNode(FVoxelGraphCompilerHelpers::GetPassthroughNode(EVoxelPinCategory::Float, BiomeMergeNode->Context, BiomeMergeNode->SourceNodes));
			...
			for (int32 BiomeIndex = 0; BiomeIndex < BiomesNum; BiomeIndex++)
			{
				...
				auto* FunctionSeparator = Compiler.AddNode(GetDefault<UVoxelNode_FunctionSeparator>()->GetCompilationNode(Compiler.Context), BiomeMergeNode);
