...
void UALSCharacterAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	if (bShouldCombatStep)
	{
		DeltaWalkTime += DeltaSeconds * 175.f;
	}
...
void UALSCharacterAnimInstance::PlayWalkingBlendAnimation()
{
	GetWorld()->GetTimerManager().SetTimer(StepResetTimer, this, &UALSCharacterAnimInstance::ResetWalkAndSetOverlay,
		0.5f, true);
	bShouldCombatStep = true;
}

void UALSCharacterAnimInstance::ResetWalkAndSetOverlay()
{
	// I need to make this fucntion set the overlay state to either the first or second pose in the montage form the waist down.
	StepNumber = 0;
	bShouldCombatStep = false;
}