...
FVoxelGraphCompiler::FVoxelGraphCompiler(const FVoxelGraphContext& Context, const TSharedRef<FVoxelGraphErrorReporter>& ErrorReporter)
	: Context(Context)
...
FVoxelGraphCompiler::FVoxelGraphCompiler(const FVoxelGraphContext& Context, UVoxelGraphGenerator* Graph)
	: FVoxelGraphCompiler(Context, MakeShared<FVoxelGraphErrorReporter>(Graph))
{
}
...
{
	TSharedRef<FVoxelGraphCompiler> Result = MakeShareable(new FVoxelGraphCompiler(Context, MakeShared<FVoxelGraphErrorReporter>(ErrorReporter, ErrorPrefix)));
...
FVoxelCompilationNode* FVoxelGraphCompiler::AddNode(UVoxelNode* Node)
{
	...
	if (!ErrorReporter.HasError())
	{
		return AddNode(Node->GetCompilationNode(Context));
	}
	else
	{
		return nullptr;
...
void FVoxelGraphCompiler::Check() const
{
	...
	for (auto& Node : Nodes)
	{
		...
		if (Node->IsSeedNode() &&
			!Node->IsA<FVoxelMacroInputOutputCompilationNode>() &&
		...
		for (auto& Pin : Node->IteratePins<EVoxelPinIter::All>())
		{
			...
			if (Pin.Direction == EVoxelPinDirection::Output && Node->IsSeedNode() && !Node->IsA<FVoxelMacroCompilationNode>() && !Node->IsA<FVoxelMacroInputOutputCompilationNode>())
			{
				ensureVoxelGraph(Pin.PinCategory == EVoxelPinCategory::Seed, Node);
			}
		}
