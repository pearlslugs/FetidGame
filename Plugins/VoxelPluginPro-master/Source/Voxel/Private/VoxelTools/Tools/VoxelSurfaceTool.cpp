...
bool UVoxelSurfaceTool::GetMaskData(bool bShowNotification, TVoxelTexture<float>& OutTexture, float& OutScaleX, float& OutScaleY)
{
	...
	{
		FString Error;
		if (!FVoxelTextureHelpers::CanCreateFromTexture(Mask.Texture, Error))
		{
			if (bShowNotification)
			{
				...
				Button.OnClick = FSimpleDelegate::CreateWeakLambda(Mask.Texture, [Mask = Mask.Texture]() { FVoxelTextureHelpers::FixTexture(Mask); });

				FVoxelMessages::ShowNotification(Notification);
			}
			return false;
		}
		OutTexture = FVoxelTextureHelpers::CreateFromTexture_Float(Mask.Texture, Mask.Channel);
	...
	else
	{
		...
		if (bNeedToRebuild)
		{
			...
			MaskGeneratorCache.VoxelTexture = FVoxelTextureHelpers::Normalize(*FloatTexture);

			const auto ColorTexture = FVoxelTextureHelpers::CreateColorTextureFromFloatTexture(MaskGeneratorCache.VoxelTexture, EVoxelRGBA::R, false);
			
			FVoxelTextureHelpers::CreateOrUpdateUTexture2D(ColorTexture, MaskGeneratorCache_RenderTexture);
		}
