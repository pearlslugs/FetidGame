...
void FVoxelReplaceSmartMinMaxPass::Apply(FVoxelGraphCompiler& Compiler)
{
	for (auto* NodeIt : Compiler.GetAllNodesCopy())
	{
		...
		{
			auto* GetRange = Compiler.AddNode(GetDefault<UVoxelNode_GetRangeAnalysis>()->GetCompilationNode(Compiler.Context), Node);
		...
		FVoxelCompilationNode* MaxOfRangeMin;
		{
			...
			MaxOfRangeMin = Compiler.AddNode(MaxVoxelNode->GetCompilationNode(Compiler.Context), Node);
		...
		for (int32 InputIndex = 0; InputIndex < NumInputs; InputIndex++)
		{
			...
			auto* LessThan = Compiler.AddNode(LessThanVoxelNode->GetCompilationNode(Compiler.Context), Node);
			...
			auto* If = Compiler.AddNode(GetDefault<UVoxelNode_IfWithDefaultToTrue>()->GetCompilationNode(Compiler.Context), Node);
			If->GetInputPin(0).LinkTo(*LastExecPin);
			If->GetInputPin(1).LinkTo(LessThan->GetOutputPin(0));

			auto* FlowMerge = Compiler.AddNode(GetDefault<UVoxelNode_FlowMerge>()->GetCompilationNode(Compiler.Context), Node);
		...
		FVoxelCompilationNode* FunctionSeparator = Compiler.AddNode(GetDefault<UVoxelNode_FunctionSeparator>()->GetCompilationNode(Compiler.Context), Node);
		...
		// Do the max of the flow merge values, and link it to the function separator input
		{
			...
			auto* Max = Compiler.AddNode(MaxVoxelNode->GetCompilationNode(Compiler.Context), Node);
			for (int32 InputIndex = 0; InputIndex < NumInputs; InputIndex++)
			{
