...
class VOXELGRAPH_API FVoxelNameVariable : public FVoxelExposedVariable
{
public:
	FVoxelNameVariable(const UVoxelExposedNode& Node, FName Name);
};

template<typename T>
class TVoxelObjectVariable : public FVoxelExposedVariable
{
public:
	TVoxelObjectVariable(const UVoxelExposedNode& Node, T* Default)
		: FVoxelExposedVariable(
			Node,
			FVoxelCppUtils::TypeToString<TWeakObjectPtr<T>>(),
			FVoxelCppUtils::SoftObjectPtrString<T>(),
			FVoxelCppUtils::ObjectDefaultString(Default))
	{
	}
};
