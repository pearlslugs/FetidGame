...
void FVoxelGraphPreview::UpdateTextures(EVoxelGraphPreviewFlags Flags)
{
	...
	if (!Data || EnumHasAnyFlags(Flags, EVoxelGraphPreviewFlags::UpdatePlaceableItems))
	{
		...
		{
			PreviewSettings.PlaceableItemManager->SetGeneratorCache(FVoxelGeneratorCache::Create(Init));
	...
	// Always do it as it's using Center
	{
		...
		if (PreviewSettings.PlaceableItemManager)
		{
			class FGraphCoordinatesProvider : public FVoxelCoordinatesProvider
			{
			public:
				const FVoxelGraphPreviewSettingsWrapper& Settings;

				explicit FGraphCoordinatesProvider(const FVoxelGraphPreviewSettingsWrapper& Settings)
					: Settings(Settings)
				{
				}

				virtual v_flt GetVoxelSize() const override
				{
					return 1;
				}
				virtual FVoxelDoubleTransform GetVoxelTransform() const override
				{
					return FTransform(FTranslationMatrix(-FVector(Settings.Center)) * FScaleMatrix(FVector(1.f / Settings.Step)));
				}

			};
			PreviewSettings.PlaceableItemManager->DrawDebug(FGraphCoordinatesProvider(Settings), *LineBatchComponent);
		}
	}
	...
	if (bComputeCosts)
	{
		...
		},[&](const TUniquePtr<FVoxelConstDataAccelerator>& Accelerator, int32 X)
		{
			for (int32 Y = 0; Y < Settings.Resolution; Y++)
			{
				...
				Values[Index] = Accelerator->GetFloatValue(FVoxelVector(Position), PreviewSettings.LODToPreview);
